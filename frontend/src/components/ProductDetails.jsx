      ...
      {artworks.map((data) => (
        <React.Fragment key={data._id}>
          ...
          <div className="grid grid-cols-2 gap-0">
            ...
            {data.images[2] && (
              <img
                src={`http://localhost:3000${data.images[2]}`}
                alt="Product Image 3"
                className="col-span-2 w-full h-auto object-cover rounded-none shadow-none pb-32"
              />
            )}
          ...
          {/* Product Information Section */}
          <div className="flex flex-col">
            <h1 className="text-4xl font-bold font-serif text-gray-800 mb-4">{data.title}</h1>
            <p className="text-xl font-semibold text-gray-700 mb-8">₹{data.price}</p>
